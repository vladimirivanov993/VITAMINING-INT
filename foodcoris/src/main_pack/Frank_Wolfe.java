...
//System.out.println();
    ...
    public static double[] getSolSimplex(double[] uk, double[][] A, double[] b, double[][] simp_A, double[] simp_b, double eps) {
        ...
        while (true) {
            gamma = 2 / (k + 2);
            uk = plus(mul(1 - gamma, uk), mul(gamma, s));
            k++;
            s = new Simplex(simp_A, simp_b, gradient(A, b, uk)).primal();
            toPrint(uk);
            System.out.println("fx=" + (mul(mul(A, uk), uk) + mul(b, uk)));
            toPrint(s);
            System.out.println("fx=" + (mul(mul(A, s), s) + mul(b, s)));
    ...
    public static double[] gM(double[][] A, double[] b, double[] c, double eps, double[][] A2, double[] b2) {
        ...
        for (double i = 0; i < c[1]; i += eps) {
            for (double j = 0; j < c[2]; j += eps) {
                ...
                if (curMax >= max && positive(minus(b2, (mul(A2, new double[]{i, j}))))) {
                    max = curMax;
                    r[0] = curMax;
                    r[1] = i;
                    r[2] = j;
...
//double[] uk={0.0,0.0};
        ...
        double[] sp = s.primal();
        System.out.print("Проверка симплекс-метода(линейные функция и ограничения): ");
        toPrint(sp);
        ...
        System.out.print("Проверка корректности применения симплекс метода к градиенту квадратичной функции: ");
        toPrint(sp2);
        ...
        System.out.print("Проверка корректности функции вычисления градиента { {4,8},{8,10} }+{7,11} в точке: {2,3}: ");
        toPrint(gradient(gA2, gb2, uk2));
        System.out.print("Решение задачи методом условного градиента с функцией (1): ");
        toPrint(getSolSimplex(uk2, gA2, gb2, A2, b2, 0.01));
        System.out.print("Решение задачи методом перебора с функцией (1): ");
        toPrint(gM(gA2, gb2, new double[]{0, 6, 6}, 0.1, A2, b2)); //eps

        double[][] gA3 = {{-40, -8}, {-8, -110}};
        double[] gb3 = {700, 111};

        System.out.print("Решение задачи методом условного градиента с функцией (2): ");
        toPrint(getSolSimplex(uk2, gA3, gb3, A2, b2, 0.01));
        System.out.print("Решение задачи методом перебора с функцией (2): ");
        toPrint(gM(gA3, gb3, new double[]{0, 6, 6}, 0.1, A2, b2)); //eps
    }
}